...
def handel(delt):
    ...
    # Droop constants [Pcontrol; Icontrol; P/Vcontrol; P/Vcontrol; I/Vcontrol; Pcontrol]
    K_d = np.array([0, 0, 3, 2.5, 0.1, 0]).reshape(-1, 1)  # K_d = 0 means constant power or current control
    ...
    # Bus type [V; I; P/V; I/V; P]
    BUS = [[Vslack, 0, 0, 0, 0, 1], 
           [Vslack, I2ref, 0, 0, 0, 2], 
           [Vslack, 0, P3ref+ V3ref * K_d[2], 0, 0, 3],
           [Vslack, 0, P4ref+ V4ref * K_d[3], 0, 0, 3], 
           [Vslack, 0, 0, I5ref+ V5ref * K_d[4], 0, 4],
           [Vslack, 0, 0, 0, P6ref, 5]]
    ...
    BUS[:, 0:1] = BUS[:, 0:1] / VB
    BUS[:, 1:2] = BUS[:, 1:2] / (SB / VB)
    BUS[:, 2:3] = BUS[:, 2:3] / SB
    BUS[:, 3:4] = BUS[:, 3:4] / (SB / VB)
    BUS[:, 4:5] = BUS[:, 4:5] / SB

    K_d[2:3] = K_d[2:3] * VB / SB  # pu conversion for P/V droop constants MW/kV
    K_d[4] = K_d[4] * VB / (SB / VB)  # pu conversion for I/V droop constants kA/kV

    # line parameters
    Line = [[1, 4, 1.0], [2, 5, 1.2], [3, 6, 1.2], [1, 6, 0.8], [1, 2, 0.8]]  # line1. line2, line3, line4, line5
    Line = np.array(Line)
    ...
    i_b = np.argwhere(type[:, 0] == 2)
    pv_b = np.argwhere(type[:, 0] == 3)
    iv_b = np.argwhere(type[:, 0] == 4)
    p_b = np.argwhere(type[:, 0] == 5)
    ...
    while (Tol > 0.00001 and Iteration <= 30):
        ...
        dI = Iini[1:2] - I[1:2]
        dPV = PVini[2:4] - (V[2:4] * K_d[2:4] + P[2:4])
        dIV = IVini[4:5] - (V[4:5] * K_d[4:5] + I[4:5])
        dP = Pini[5:6] - P[5:6]  # delta
        ...
        Nh1 = np.hstack((NII, NIPV, NIIV, NIP))
        Nh2 = np.hstack((NPVI, NPVPV, NPVIV, NPVP))
        Nh3 = np.hstack((NIVI, NIVPV, NIVIV, NIVP))
        Nh4 = np.hstack((NPI, NPPV, NPIV, NPP))
        ...
        dfunction = np.vstack((dI, dPV, dIV, dP))
        # print(dfunction)
    ...
    P_B2 = BUS[1, 1] * (SB / VB) * V[1]  # I control
    P_B3 = BUS[2, 2] * SB - V[2] * K_d[2] / (VB / SB)  # P/V control
    P_B4 = BUS[3, 2] * SB - V[3] * K_d[3] / (VB / SB)  # P/V control
    P_B5 = (BUS[4, 3] * (SB / VB) - V[4] * K_d[4] / (VB / (SB / VB))) * V[4]  # I/V control
    P_B6 = BUS[5, 4] * SB  # P control
...
def find_Peak():
    ...
    step = 0.0001
    flag_all = 0
    max_iterations = 50  # maximum allowable search times
    count = 0
    ...
    while flag_all != 1 and count < max_iterations:
        ...
        # df.to_excel('Binary_results_slackbus5.xlsx', index=False)
...
# print(f"Results have been saved to 'Binary_results_slackbus5.xlsx'")
