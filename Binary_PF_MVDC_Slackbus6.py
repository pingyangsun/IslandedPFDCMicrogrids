...
def handel(delt):
    ...
    # Droop constants [ Icontrol; PVcontrol; PVcontrol; IVcontrol; Pcontrol; Pcontrol]
    K_d = np.array([0, 3, 2.5, 0.1, 0, 0]).reshape(-1, 1)  # K_d = 0 means constant power or current control
    ...
    # Bus type [V; P/V; I/V; P; ~]
    BUS = [[Vslack, 0, 0, 0, 0, 1], 
           [Vslack, P2ref+ V2ref * K_d[1], 0, 0, 0, 2], 
           [Vslack, P3ref + V3ref * K_d[2], 0, 0, 0, 2],
           [Vslack, 0, I4ref + V4ref * K_d[3], 0, 0, 3], 
           [Vslack, 0, 0, P5ref, 0, 4],
           [Vslack, 0, 0, P6ref, 0, 4]]
    ...
    K_d[1:2] = K_d[1:2] * VB / SB  # pu conversion for P/V droop constants MW/kV
    K_d[3] = K_d[3] * VB / (SB / VB)  # pu conversion for I/V droop constants kA/kV

    # line parameters
    Line = [[1, 4, 1.2], [2, 5, 1.2], [3, 6, 1.0], [1, 6, 0.8], [5, 6, 0.8]]  # line1. line2, line3, line4, line5
    Line = np.array(Line)
    ...
    pv_b = np.argwhere(type[:, 0] == 2)
    iv_b = np.argwhere(type[:, 0] == 3)
    p_b = np.argwhere(type[:, 0] == 4)
    # i_b = np.argwhere(type[:, 0] == 5)
    ...
    while (Tol > 0.00001 and Iteration <= 30):
        ...
        dP = Pini[4:6] - P[4:6]  # delta
        # dI = Iini[5:6] - I[5:6]
        dPV = PVini[1:3] - (V[1:3] * K_d[1:3] + P[1:3])
        dIV = IVini[3:4] - (V[3:4] * K_d[3:4] + I[3:4])
        ...
        # Nh1 = np.hstack((NPVPV, NPVIV, NPVP, NPVI))
        # Nh2 = np.hstack((NIVPV, NIVIV, NIVP, NIVI))
        # Nh3 = np.hstack((NPPV, NPIV, NPP, NPI))
        # Nh4 = np.hstack((NIPV, NIIV, NIP, NII))

        Nh1 = np.hstack((NPVPV, NPVIV, NPVP))
        Nh2 = np.hstack((NIVPV, NIVIV, NIVP))
        Nh3 = np.hstack((NPPV, NPIV, NPP))
        # Nh4 = np.hstack((NIPV, NIIV, NIP, NII))

        J = np.vstack((Nh1, Nh2, Nh3))
        dfunction = np.vstack((dPV, dIV, dP))
        # print(dfunction)
    ...
    P_B5 = BUS[4, 3] * SB  # P control
    P_B6 = BUS[5, 3] * SB  # P control
    P_B2 = BUS[1, 1] * SB - V[1] * K_d[1] / (VB / SB)  # P/V control
    P_B3 = BUS[2, 1] * SB - V[2] * K_d[2] / (VB / SB)  # P/V control
    P_B4 = (BUS[3, 2] * (SB / VB) - V[3] * K_d[3] / (VB / (SB / VB))) * V[3]  # I/V control
...
def find_Peak():
    ...
    while flag_all != 1 and count < max_iterations:
        ...
        # df.to_excel('Binary_results_slackbus6.xlsx', index=False)
...
# print(f"Results have been saved to 'Binary_results_slackbus6.xlsx'")
