...
def handel(delt):
    ...
    K_d = np.array([0, 0, 0, 3, 2.5, 0.1]).reshape(-1, 1) # K_d = 0 means constant power or current control
    ...
    BUS=[[Vslack,0,0,0,0,1],
         ...
         [Vslack,0,I3ref,0,0,3],
         [Vslack,0,0,P4ref+V4ref*K_d[3],0,4],
    ...
    K_d[3:4] = K_d[3:4]*VB/SB      # pu conversion for P/V droop constants MW/kV
    K_d[5] = K_d[5]*VB/(SB/VB) # pu conversion for I/V droop constants kA/kV

    # line parameters
    Line=[[1,2,0.8],[2,3,0.8],[1,4,1.2],[2,5,1.0],[3,6,1.2]] # line1. line2, line3, line4, line5
    Line=np.array(Line)
    ...
    while(Tol > 0.00001 and Iteration <= 30):
        ...
        dI = Iini[2:3] - I[2:3] 
        dPV = PVini[3:5] - (V[3:5]*K_d[3:5] + P[3:5])
        dIV = IVini[5:6] - (V[5:6]*K_d[5:6] + I[5:6])
...
def find_Peak():
    ...
    while flag_all != 1 and count < max_iterations:
        ...
        # df.to_excel('Binary_results_slackbus4.xlsx', index=False)
...
# print(f"Results have been saved to 'Binary_results_slackbus4.xlsx'")
